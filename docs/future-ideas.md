...
## Exporting the data — "what happens if I uninstall?"

**Asked for (Sep 2026, r/Roll20 launch thread):** the very first question the
launch post got, from a stranger, was *"If you use it and then remove it,
what happens to the data?"* The answer given in-thread is the true one: the
handouts live in the campaign, so uninstalling, reinstalling or never
installing again loses nothing, and the data can be read straight out of the
`PT-…` handouts by hand. The reply promised a tool to make that easier to
parse, and this is that promise.

**Treat this as a trust feature, not a convenience one.** It was the first
thing anybody asked, unprompted, before any question about what the tool
actually does. The screenshot posted alongside that answer — a raw `PT-…`
handout body, a wall of dense JSON — makes the case better than any
argument could: the data genuinely survives, and it is genuinely unreadable.
Half an answer. An export turns "you *can* get your data out" into "here is
your data", which is the difference between a claim and a demonstration.

**What it would take.** The read side already exists — the storage layer
parses every one of these docs today, so an export is a serialiser over
data structures already in memory, not new Roll20 work.

**The open questions, hardest first:**

1. **The DM/player boundary is the whole problem, not a detail.** The
   storage design deliberately splits data by who Roll20's server will
   deliver it to: a bag handout's *entire* body reaches every player
   (gmnotes included — see the hard rule above `stashObscuredTruth`), while
   hidden bags and the true stats of obscured items live only in the GM
   index handout, and DM-secret actions live only in the GM log. An export
   must inherit that split exactly. A player-run export can contain only
   what that player can already see; a DM-run export can reasonably contain
   everything, but then the resulting file is a plaintext spoiler sheet
   sitting in someone's Downloads folder. Getting this wrong leaks prepped
   loot, which is the one failure this tool must never have.

2. **Format.** The asker's use case is "parse it"; the more common use case
   is "read it" or "put it back in a spreadsheet". Those want different
   things. CSV per bag suits the spreadsheet crowd and the tool's own
   origin story; a single readable JSON document suits the parsers; a
   Markdown table suits someone pasting the party's haul into Discord.
   Probably more than one, but not all three at once.

3. **How the file actually gets out, and at what cost.** The manifest
   currently requests **no permissions of any kind** — that is a selling
   point in the store listing, the privacy policy and the reviewer notes,
   and adding `downloads` would spend it, trigger re-review on both stores,
   and show existing users a new permission warning. A Blob URL plus a
   synthetic `<a download>` click from the page context should avoid the
   permission entirely. **This is the thing to test first**, because it is
   the only question whose answer could change the shape of the feature:
   Roll20's own Content-Security-Policy may block `blob:` navigation on the
   editor page. If it does, the fallback is rendering the export into a
   textarea for manual copy — less slick, still a complete answer to the
   question that was asked, and still zero permissions.

4. **Does the log come with it?** The activity log is the part with no
   equivalent anywhere else — an item list can be rebuilt from sheets and
   memory, but "who took the rope, and when" cannot. It is also the bulkiest
   part (capped at 4000 entries) and the least useful in a spreadsheet.
   Probably a separate export rather than a column.

**What would settle it:** test the Blob-download path on a real Roll20
editor page first — everything else is a choice, that one is a constraint.
Then pick the narrowest version that answers the original question: a DM-run
export of the bags they can see, as CSV, with the boundary in (1) enforced
by reading through the same code paths the panel already uses rather than by
a new filter written for the exporter.
