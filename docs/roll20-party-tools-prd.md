...
- INV-16d — **The obscuring itself must be invisible to players (added v0.5.1, found in play testing).** It is not enough to withhold the true data: players must have no cue that an item is *special at all*. An obscured item must appear in the player's panel exactly as an ordinary item with a plain name — no marker, no highlight, no badge, and no flag in the data their client receives. UI-7's "make hidden content unmistakable" applies to the **DM's view only**; applied to a player's view it defeats the requirement it is meant to serve. Consequence: obscured-ness is stored solely in the GM-only handout and never as a field on the shared item record.
